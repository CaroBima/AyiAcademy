...
function App() {
  ...
  const handleEnviarClick = () => {
    console.log("manda el click " + nombre);
    //aca me falta activar el componente hijo
      };
    ...
    <div className="row justify-content-md-center mt-5">
      <div className="col-md-6 text-center mt-5">
        <div className="form-floating mb-3">
          <input type="text" className="form-control" id="nombreInput" onChange={handleInputChange}/>
          <label htmlFor="nombreInput">Nombre</label>
        </div>
        <button className="btn btn-outline-success btn-lg col-md-6" type="submit" onClick={handleEnviarClick}>Enviar</button>
      </div>
    </div>
  );
}
